...
async def get_speciality(request: Request, speciality: str):
    ...
    try:
        ...
        data = (
            db.query(Doctors).filter(Doctors.speciality.ilike(f"%{speciality}")).all()
        )
        return templates.TemplateResponse(
            "doctors.html", {"request": request, "data": data, "speciality": speciality}
        )
    finally:
        db.close()
...
async def get_speciality(request: Request, speciality: str):
    ...
    try:
        ...
        return templates.TemplateResponse(
            "record_doctors.html",
            {"request": request, "data": data, "speciality": speciality},
        )
    finally:
        db.close()
...
async def registration_spec(data=Body(), db: Session = Depends(get_db)):
    ...
    specialists_exists = (
        ...
        .filter(
            (Specialists.email == specialists.email)
            | (Specialists.username == specialists.username)
            | (Specialists.speccode == specialists.speccode)
        )
        .first()
    )
    if specialists_exists:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Код специалиста не найден"
        )
...
async def login_spec(data: dict = Body(), db: Session = Depends(get_db)):
    ...
    specialists_exists = (
        db.query(Specialists)
        .filter((Specialists.username == username) & (Specialists.password == password))
        .first()
    )
...
async def registration_patient(data=Body(), db: Session = Depends(get_db)):
    ...
    patients_exists = (
        ...
        .filter(
            ...
            | (Patients.username == patients.username)
        )
        .first()
    )
    if patients_exists:
...
async def login_patient(data: dict = Body(), db: Session = Depends(get_db)):
    ...
    patient_exists = (
        db.query(Patients)
        .filter((Patients.username == username) & (Patients.password == password))
        .first()
    )
